...
/**
 * This class contains methods to allow you to use the assert methods More steps
 * examples here:
...
public class AssertionSteps {
	...
	@Then("^the user should\\s*((?:not)?)\\s+see page title as \"(.+)\"$")
	...
	/**
	 * Check if an option with a certain value is (selected/unselected) on a
	 * drop-down
	 */
	@Then("^option with value \"(.*?)\" from dropdown having (.+) \"(.*?)\" should be (selected|unselected)$")
	public void checkDropdownOptionSelected(String value, String type, String type2, String key, String select)
			throws Exception {
